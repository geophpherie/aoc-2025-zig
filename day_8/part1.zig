...
pub fn main() !void {
    ...
    const result = try puzzle(allocator, input, 1000);
...
fn puzzle(allocator: std.mem.Allocator, input: []const u8, numPairs : usize) !u64 {
    ...
    // update elements
    // look at array
    for (pairs.items[0..numPairs]) |pair| {
...
test "test input" {
    ...
    const result = try puzzle(allocator, input, 10);
