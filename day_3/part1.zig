    ...
    while (lines.next()) |line| {
        ...
        for (0..num_digits) |digit| {
            for ("987654321") |c| {
                ...
                std.debug.print("Checking {c} in {s} for {d}\n", .{ c, line, digit });
                var start_ind: usize = undefined;
                var end_ind: usize = undefined;
                ...
                if (digit == 0) {
                    ...
                    end_ind = line.len - (num_digits - digit) + 1;
                }
                ...
                if (idx) |i| {
                    std.debug.print("Largest First At: {}\n", .{i});
                    str_num[digit] = c;
                    str_ind[digit] = i;
                    break;
                }
        ...
        const new_joltage = try std.fmt.parseInt(u8, &str_num, 10);
